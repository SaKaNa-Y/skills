...
After all eligible searches settle, present the batch and follow [issue capture](references/issue-capture.md) for exact drafts and Tracker Write Approval. Summarize every disposition using the [outcome reporting contract](references/issue-capture.md#report-the-outcome); Reused records need no mutation draft or approval. Creating or updating an issue or local Markdown record requires the user's approval of the concrete mutation. Keep unapproved drafts in the conversation.
