...
If the target itself must be built or configured, use a disposable copy or worktree when practical. Keep the original checkout's tracked files unchanged and preserve existing authorization boundaries for dependency installation, network access, credentials, or external state.
